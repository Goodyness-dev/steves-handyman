...
export default function LocationHoursSection({ onOpenWizard }) {
  ...
  return (
    <section id="location" className="py-24 sm:py-32 bg-black dark:bg-onyx-950 text-white relative border-t border-neutral-900 blueprint-grid" aria-labelledby="location-heading">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          <h2 id="location-heading" className="text-3xl sm:text-5xl font-black font-heading text-white tracking-tight">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          {/* Hours Card */}
          <div className="lg:col-span-5 card-thick rounded-3xl p-8 sm:p-10 bg-gradient-to-b from-neutral-900 to-onyx-900 border-2 border-neutral-800 flex flex-col justify-between">
            <div>
              {/* Open/Closed Badge */}
              <div className="flex items-center justify-between p-4 rounded-2xl bg-black/50 border border-neutral-800 mb-6">
                <div className="flex items-center space-x-3.5">
                  <span className={`w-3.5 h-3.5 rounded-full ${shopOpen ? 'bg-emerald-400 animate-pulse' : 'bg-neutral-500'}`} />
                  <div>
                    <span className={`font-heading font-black text-base block ${shopOpen ? 'text-emerald-400' : 'text-neutral-300'}`}>
                      {shopOpen ? 'Technician On Duty Today' : 'After Hours • Quotes Active'}
                    </span>
                    <span className="text-xs text-neutral-400">Today is {currentDayName}</span>
                  </div>
                </div>
              </div>
                    ...
                    return (
                      <div
                        ...
                        className={`py-2.5 px-3 flex justify-between items-center rounded-xl transition-colors ${
                          isToday ? 'bg-white/10 font-bold text-white' : 'text-neutral-400'
                        }`}
                      >
                        <span className="flex items-center gap-2">
                          {h.day}
                          {isToday && (
                            <span className="text-[10px] uppercase px-2 py-0.5 rounded-md bg-white text-black font-black">
                              Today
                            </span>
                          )}
                        </span>
                        <div className="text-right">
                          <span className={h.open === 'Closed' ? 'text-neutral-500 font-medium' : 'text-neutral-200'}>
          ...
          <div className="lg:col-span-7 card-thick rounded-3xl p-8 sm:p-10 bg-gradient-to-b from-neutral-900 to-onyx-900 border-2 border-neutral-800 flex flex-col justify-between">
            <div>
              ...
              <h3 className="text-2xl sm:text-3xl font-black font-heading text-white mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
                {serviceTowns.map((town) => (
                  <div key={town} className="p-3 rounded-xl bg-black/40 border border-neutral-800 flex items-center gap-2.5 text-xs sm:text-sm text-neutral-300 font-medium">
            ...
            <div className="pt-6 border-t border-neutral-800 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <button
                ...
                className="w-full sm:w-auto px-6 py-3 rounded-full bg-white text-black font-heading font-black text-xs uppercase tracking-wider hover:bg-neutral-200 transition shadow-lg active:scale-95"
              >
                CHECK MY ADDRESS ↗
              </button>
            </div>
